...
/**
 * @class UpdateOrganizationPolicyHandler
 * @implements IPolicyHandler
 * @description
 * 组织“更新组织”权限策略处理器，用于判断当前用户是否具备更新指定组织（Update Organization）的能力。
 *
 * 代码原理与机制说明：
 * 1. 实现IPolicyHandler接口，统一策略处理器规范，便于与CASL能力工厂集成，实现灵活的权限校验。
 * 2. handle方法接收当前用户的AppAbility实例（由CASL能力工厂动态生成），
 *    并调用ability.can(Actions.Update, Subjects.Organization)判断用户是否拥有“更新组织”权限。
 * 3. 该策略通常用于组织管理员或有更新权限的用户，结合PoliciesGuard与@CheckPolicies装饰器，
